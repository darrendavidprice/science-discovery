...
#
def get_ratio_1D (data1, data2, bins, weights1=None, weights2=None, as_lines=False, normed=True, rogue_both=np.nan, rogue_den=np.inf) :
    ...
    frac_EZ1   = safe_divide(EZ1, Z1, rogue_both=rogue_both, rogue_den=rogue_den)
    frac_EZ2   = safe_divide(EZ2, Z2, rogue_both=rogue_both, rogue_den=rogue_den)
    ratio      = safe_divide(Z2 , Z1, rogue_both=rogue_both, rogue_den=rogue_den)
...
#  Get 1D pull
#
def get_pull_1D (data_x1, data_x2, bins_x, weights1=None, weights2=None, as_lines=False, rogue_both=np.nan, rogue_den=np.inf) :
    ...
    return X, safe_divide(num, den, rogue_both=rogue_both, rogue_den=rogue_den)


#  Get 2D ratio between two histograms
#
def get_ratio_2D (data_x1, data_y1, data_x2, data_y2, bins_x, bins_y, weights1=None, weights2=None, rogue_both=np.nan, rogue_den=np.inf) :
...
#  Get 2D pull
#
def get_pull_2D (data_x1, data_y1, data_x2, data_y2, bins_x, bins_y, weights1=None, weights2=None, rogue_both=np.nan, rogue_den=np.inf) :
    X, Y, Z1, EZ1 = bin_data_2D(data_x1, data_y1, bins_x, bins_y, weights1)
    X, Y, Z2, EZ2 = bin_data_2D(data_x2, data_y2, bins_x, bins_y, weights2)
    num = Z2 - Z1
    den = np.sqrt(EZ1*EZ1 + EZ2*EZ2)
    return X, Y, safe_divide(num, den, rogue_both=rogue_both, rogue_den=rogue_den)
...
#
def safe_divide (num, den, rogue_both=0., rogue_den=np.inf) :
    assert len(num) == len(den), f"len(num) ({len(num)}) != len(den) ({len(den)})"
    if type(num[0]) in [np.ndarray, list] :
        return np.array([safe_divide(row_num, row_den, rogue_both=rogue_both, rogue_den=rogue_den) for row_num, row_den in zip(num, den)])
    return np.array([x/y if yAbs>0 else rogue_both if x==0 else rogue_den for x,y,yAbs in zip(num, den, np.fabs(den))])
