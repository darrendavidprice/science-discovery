...
def do_general_setup (prog_name="do_general_setup()") :
	global num_scan_params, meas_dist, SM_model_dist, SM_pred_dist, BSM_input_dists, BSM_scan_dists, param_grid
	#
	#  parse arguments
	#
	...
	SM_pred_dist    = inputs.get_SM_dist  (name="get_limits.py::SM::pred" , key="experimental")
	...
	utils.info(prog_name, f"SM experimental distribution is {SM_pred_dist}")
...
#
def get_toy_spread_of_limits(SM_pred_dist, SM_model_dist, BSM_scan_dists, n_toys, target_coverage=None, tag=None) :
	global param_grid
...
def main () :
	...
	grid_of_pred_CL = stats.get_CL_across_grid (SM_pred_dist, BSM_scan_dists, SM_dist=SM_model_dist)
	...
	SM_toy_limits, SM_coverage_grid = get_toy_spread_of_limits(SM_pred_dist, SM_model_dist, BSM_scan_dists, n_toys, tag=tag)
	if num_scan_params == 1 :
		utils.info("get_limits.py", f"Median {100.*target_coverage:.2f}% limit of SM toys is {SM_toy_limits[int(0.5*n_toys)]:.0f}")
		utils.info("get_limits.py", f"16th percentile {100.*target_coverage:.2f}% limit of SM toys is {SM_toy_limits[int(0.16*n_toys)]:.0f}")
		utils.info("get_limits.py", f"84th percentile {100.*target_coverage:.2f}% limit of SM toys is {SM_toy_limits[int(0.84*n_toys)]:.0f}")
	#
	# plot
	#
	utils.set_mpl_style()
	...
	plt.contourf(SM_coverage_grid.axes[0], SM_coverage_grid.axes[1], SM_coverage_grid.values.transpose(), [0.16, 0.84], linestyles=None, colors=["gold"])
	plt.contour(SM_coverage_grid.axes[0], SM_coverage_grid.axes[1], SM_coverage_grid.values.transpose(), [0.16, 0.5, 0.84], linestyles="dashed", colors=["darkorange", "darkblue", "darkorange"], linewidths=1)
	plt.contour(grid_of_pred_CL.axes[0], grid_of_pred_CL.axes[1], grid_of_pred_CL.values.transpose(), [0.05], colors="green")
	plt.contour(grid_of_obs_CL.axes[0], grid_of_obs_CL.axes[1], grid_of_obs_CL.values.transpose(), [0.05], colors="purple")
	...
	ax.tick_params(which="both", direction="in", color="gray")
	...
	for label in utils.string_to_object( config.config.get("GET_LIMITS", "Labels") ) :
	...
	plt.legend( [Line2D([0], [0], color="purple", lw=2), 
				 ...
				 Line2D([0], [0], color="darkorange"  , linestyle="dashed", lw=1)],
				["Obs. (95% $CL_s$)",
				 "Exp. (95% $CL_s$)",
				 "SM toys: median",
				 "SM toys: 68% coverage"],
