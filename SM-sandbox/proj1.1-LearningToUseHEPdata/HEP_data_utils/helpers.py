	...
	for i in range(0,len(indep_vars_[0]["values"])) :
		...
		if bin.get("value",None) != None :
			...
			if bin["low"] != distribution_._bin_values[i] :
				msg.fatal("HEP_data_utils.helpers.set_1D_bins","Bin entry {0} for distribution {1} is not continuous from the previous bin which ended at {2}".format(bin,distribution_._description,distribution_._bin_values[i]))
			distribution_._bin_values[i+1] = bin["high"]
		else :
			msg.fatal("HEP_data_utils.helpers.set_1D_bins","Could not interpret bin entry {0} for distribution {1}".format(bin,distribution_._description))
...
def load_yaml_file ( datum_ , dataset_ , path_ ) :
	...
	if len(indep_vars) < 3 :
		load_distributions_from_yaml ( dataset_ , dep_vars , indep_vars , path_ , n_dim_=len(indep_vars) , metadata_global_=datum_ , metadata_local_=data[0] )
	else :
...
def load_submission_file ( dataset_ , path_ , fname_ = "" ) :
	...
	for idx in range(1,len(data)) :
		...
		load_yaml_file(datum,dataset_,filename)
