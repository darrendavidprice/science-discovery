...
def generate_ring (**kwargs) :
    ...
    for idx_x in range(num_pixels) :
        for idx_y in range(num_pixels) :
            ...
            d   = 1 - np.fabs(d-1)
            if d < 0 : continue
            z[idx_x, idx_y, 0] = z[idx_x, idx_y, 0] + np.random.normal(100., 25.)*d*d
    if kwargs.get("show", False) :
        plt.imshow(z)
        plt.title(f"{momentum:.0f} GeV at ({vertex[0]:.2f}, {vertex[1]:.2f})")
        plt.show()
    return (vertex[0], vertex[1], momentum, z)
...
def generate_shower (**kwargs) :
    ...
    for idx_x in range(num_pixels) :
        for idx_y in range(num_pixels) :
            ...
            if d > 1 : continue
            d = 1 - d
            z[idx_x, idx_y, 0] = z[idx_x, idx_y, 0] + np.random.normal(70., 25.)*d
    if kwargs.get("show", False) :
        plt.imshow(z)
        plt.title(f"{momentum:.0f} GeV at ({vertex[0]:.2f}, {vertex[1]:.2f})")
        plt.show()
