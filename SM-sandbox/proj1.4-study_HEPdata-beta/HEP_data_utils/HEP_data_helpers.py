...
def open_yaml_file ( path_ ) :
	...
	try :
		for datum in yaml.safe_load_all(yaml_file) :
			msg.info("HEP_data_utils.HEP_data_helpers.open_yaml_file","yaml file opened with entries:",verbose_level=2)
			msg.check_verbosity_and_print(yaml.safe_dump(datum),verbose_level=2)
			data.append(datum)
	except yaml.YAMLError as exc :
		print ( exc )
		msg.fatal("HEP_data_utils.HEP_data_helpers.open_yaml_file","Exception thrown when opening the yaml file (see previous messages)")
	return data
...
def get_error_from_yaml_map ( dep_var_ , error_ , pt_idx_ , err_idx_=0 ) :
	...
	if "symerror" in error_ :
		if key not in dep_var_._symerrors :
			msg.info("HEP_data_utils.HEP_data_helpers.get_error_from_yaml_map","Creating symmetric error {0} with length {1}".format(key,len(dep_var_)),verbose_level=2)
		...
		if key not in dep_var_._asymerrors_up :
			msg.info("HEP_data_utils.HEP_data_helpers.get_error_from_yaml_map","Creating asymmetric error {0} with length {1}".format(key,len(dep_var_)),verbose_level=2)
			dep_var_._asymerrors_up[key] = np.zeros(shape=(len(dep_var_)))
			dep_var_._asymerrors_dn[key] = np.zeros(shape=(len(dep_var_)))
		if "plus" not in err_asymm : msg.error("HEP_data_utils.HEP_data_helpers.get_error_from_yaml_map","No entry named \"plus\" for error \"asymerror\"")
		else : dep_var_._asymerrors_up[key][pt_idx_] = float(err_asymm["plus"])
		if "minus" not in err_asymm : msg.error("HEP_data_utils.HEP_data_helpers.get_error_from_yaml_map","No entry named \"minus\" for error \"asymerror\"")
		else : dep_var_._asymerrors_dn[key][pt_idx_] = float(err_asymm["minus"])
	else :
		print(yaml.safe_dump(error_))
		msg.error("HEP_data_utils.HEP_data_helpers.get_error_from_yaml_map","map does not have an entry called symerror or asymerror")
	return key
...
def get_dependent_variable_from_yaml_map ( dep_var_ , path_="unknown" ) :
	...
	if dep_qualifiers :
		if type(dep_qualifiers) is not list :
			msg.error("HEP_data_utils.HEP_data_helpers.get_dependent_variable_from_yaml_map","Dependent variable {0} in file {1} has qualifiers stored in type {2} where I was expecting a list".format(dep_var._name,path_,type(dep_qualifiers)),verbose_level=0)
		else :
	...
	for entry in dep_var_.get("values",[]) :
		try : val = float(entry["value"])
			...
			msg.error("HEP_data_utils.HEP_data_helpers.get_dependent_variable_from_yaml_map","ValueError ({0}) when loading dependent variable {1} from file {2}".format(exc,dep_var._name,path_),verbose_level=1)
			val = None
		except KeyError as exc :
			msg.error("HEP_data_utils.HEP_data_helpers.get_dependent_variable_from_yaml_map","KeyError ({0}) when loading dependent variable {1} from file {2}".format(exc,dep_var._name,path_),verbose_level=1)
			val = None
...
def reformat_2D_bins_as_matrix_using_labels ( dist_2D_ ) :
	if type(dist_2D_) != HEPDataTable :
		msg.fatal("HEP_data_utils.HEP_data_helpers.reformat_2D_bins_as_matrix_using_labels","argument must be of type HEPDataTable")
	dep_var = dist_2D_._dep_var
	if len(dist_2D_._indep_vars) != 2 :
		msg.fatal("HEP_data_utils.HEP_data_helpers.reformat_2D_bins_as_matrix_using_labels","HEPDataTable {0} has {1} independent_variable where 2 are required".format(dep_var._name,len(dist_2D_._indep_vars)))
	n_vals = len(dep_var)
	...
	if n_vals == old_n_bins_x*old_n_bins_y :
		...
		for key, values in dist_2D_._asymerrors_dn.items() :
			...
			dep_var._asymerrors_dn[key] = new_error
		return
	msg.warning("HEP_data_utils.HEP_data_helpers.reformat_2D_bins_as_matrix_using_labels","HEPDataTable {0} is not a matrix... assuming it is a non-factorisable distribution and returning with nothing done".format(dep_var._name),verbose_level=0)
	return
...
def load_distribution_from_yaml ( dataset_ , dep_var_ , indep_vars_ , path_ , **kwargs ) :
	...
	if not is_valid :
		msg.error("HEP_data_utils.HEP_data_helpers.load_distribution_from_yaml","Error occured when loading table {0} from file {1}... returning with nothing done.".format(key,path_))
		msg.error("HEP_data_utils.HEP_data_helpers.load_distribution_from_yaml",">>>>>>>>>>>>>>")
		msg.error("HEP_data_utils.HEP_data_helpers.load_distribution_from_yaml",validity_message)
		msg.error("HEP_data_utils.HEP_data_helpers.load_distribution_from_yaml","<<<<<<<<<<<<<<")
		return
...
def load_yaml_file ( dataset_ , path_ , **kwargs ) :
	msg.info("HEP_data_utils.HEP_data_helpers.load_yaml_file","Opening yaml file {0}".format(path_),verbose_level=0)
	data = open_yaml_file(path_)
	if len(data) != 1 :
		msg.error("HEP_data_utils.HEP_data_helpers.load_yaml_file","{0} contains {1} tables where I was expecting 1... is the file format correct?".format(path_,len(data)))
		return
	dep_vars = data[0].get("dependent_variables",None)
	if dep_vars is None :
		msg.fatal("HEP_data_utils.HEP_data_helpers.load_yaml_file","{0} contains no dependent_variables as required".format(path_))
		return
	indep_vars = data[0].get("independent_variables",None)
	if indep_vars is None :
		msg.fatal("HEP_data_utils.HEP_data_helpers.load_yaml_file","{0} contains no independent_variables as required".format(path_))
		return
...
def load_submission_file ( dataset_ , path_ ,  **kwargs ) :
	...
	for idx in range(len(data)) :
		...
		if not os.path.isfile(data_file) :
			msg.error("HEP_data_utils.HEP_data_helpers.load_submission_file","Submission file asks for a yaml file called {0} but none exists".format(data_file))
			return
...
def load_all_yaml_files ( dataset_ , path_ , **kwargs ) :
	...
	if hlp.is_directory(path_) :
		...
		if len(yaml_files) == 0 :
			msg.error("HEP_data_utils.HEP_data_helpers.load_all_yaml_files","Directory {0} has no yaml files... returning with nothing done.".format(path_),verbose_level=-1)
			return
		for f in yaml_files : load_all_yaml_files(dataset_,f,**kwargs)
		return
	if not hlp.is_yaml_file(path_) :
		msg.error("HEP_data_utils.HEP_data_helpers.load_all_yaml_files","Path {0} is not a directory or yaml file... returning with nothing done.".format(path_),verbose_level=-1)
		return
...
def load_yaml_files_from_list ( dataset_ , dir_ , **kwargs ) :
	if hlp.is_directory(dir_) :
		for filename in [ dir_+"/"+f for f in os.listdir(dir_) if is_yaml_file(f) ] :
			msg.info("HEP_data_utils.HEP_data_helpers.load_yaml_files_from_list","Opening yaml file {0}".format(filename),verbose_level=0)
		...
		msg.error("HEP_data_utils.HEP_data_helpers.load_yaml_files_from_list","Input {0} is neither a directory nor a list... returning with nothing done".format(dir_),verbose_level=-1)


#  Brief: return True if two tables have the same central values
def do_central_values_look_similar ( table1 , table2 , **kwargs ) :
	return hlp.do_arrays_look_similar ( table1._dep_var._values , table2._dep_var._values , **kwargs )
...
def do_bins_look_similar ( table1 , table2 , **kwargs ) :
	...
	for var_idx1 in range(table1.n_indep_vars()) :
		...
		for var_idx2 in range(table2.n_indep_vars()) :
			...
			if not hlp.do_arrays_look_similar(indep_var_1._bin_centers,indep_var_2._bin_centers) : continue
			if not hlp.do_arrays_look_similar(indep_var_1._bin_widths_lo,indep_var_2._bin_widths_lo,value_margin=1e-3,zero_margin=1e-5) : continue
			if not hlp.do_arrays_look_similar(indep_var_1._bin_widths_hi,indep_var_2._bin_widths_hi,value_margin=1e-3,zero_margin=1e-5) : continue
			match = True
		bins_match.append(match)
	return bins_match


#  Brief: return True only if all bins are identical
