...
def load_distribution_from_ROOT_Table ( dataset_ , key_ , table_ , **kwargs ) :
	...
	if not is_valid :
		msg.error("HEP_data_utils.ROOT_helpers.load_distribution_from_ROOT_Table","Error occured when loading table {0}... returning with nothing done.".format(key_))
		msg.error("HEP_data_utils.ROOT_helpers.load_distribution_from_ROOT_Table",">>>>>>>>>>>>>>")
		msg.error("HEP_data_utils.ROOT_helpers.load_distribution_from_ROOT_Table",validity_message)
		msg.error("HEP_data_utils.ROOT_helpers.load_distribution_from_ROOT_Table","<<<<<<<<<<<<<<")
		return
	# Add to dataset
...
def load_root_file ( dataset_ , path_ , **kwargs) :
	msg.info("HEP_data_utils.ROOT_helpers.load_root_file","Opening root file {0}".format(path_),verbose_level=0)
