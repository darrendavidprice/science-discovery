...
#  Disbale eager execution so first training epoch does no randomly take forever in tf2 (?!)
#
...
def create_discrete_density_keras_model (name, **kwargs) :
    ...
    if num_observables_in > 0 :
        observables_input = Input((num_observables_in,))
        model_observables = Dense      (3*num_observables_in, kernel_initializer=kernel_initializer, bias_initializer=bias_initializer)(observables_input)    
        model_observables = LeakyReLU  (0.2                 )(model_observables)
        model             = Concatenate(   )([model_conditions, model_observables])
    else :
        model = model_conditions
    model = Dense      (3*num_outputs, kernel_initializer=kernel_initializer, bias_initializer=bias_initializer)(model)
    model = LeakyReLU  (0.2          )(model)
    model = Dense      (2*num_outputs, kernel_initializer=kernel_initializer, bias_initializer=bias_initializer)(model)
    model = LeakyReLU  (0.2          )(model)
    
    #normalise_to_unity = lambda x : x / K.sum(x)
    model = Dense (num_outputs, activation="softmax", kernel_initializer=kernel_initializer, bias_initializer=bias_initializer)(model)
...
#  Brief:  return named model which returns the log-likelihood for set of datapoints using the DensityModel model
#
def create_evaluator_model_for_density_models (name, model, num_gauss) :
    density_models = [x.model for x in model.likelihood_models]
    assert len(density_models) > 0
    num_conditions  = density_models[0].inputs[0].shape[1]
    num_observables = len(density_models)
    #
    #  Create input layers for conditions and observables
    #
    condition_input_layer = Input((num_conditions ,), name=f"{name}_condition_input" )
    obs_input_layer       = Input((num_observables,), name=f"{name}_observable_input")
    #
    #  Create a list of layers which calculate the logL for each observable
    #
    logL_layers = []
    for i, mod in enumerate(density_models) :
        model_inputs = [condition_input_layer]
        if i > 0 :
            cond_obs_split_layer = Lambda(lambda x : x[:, :i], name=f"{name}_split_observable_below{i}")(obs_input_layer)
            model_inputs.append(cond_obs_split_layer)
        params     = mod(model_inputs)
        eval_p     = Lambda(lambda x : x[:, i]                           , name=f"{name}_split_observable_eq{i}")(obs_input_layer)
        logL_layer = Lambda(lambda x : K_get_log_likelihood(x, num_gauss), name=f"{name}_logL_observable{i}"    )([eval_p, params])
...
def K_dataset_likelihood (x, params, num_gauss) :
    ...
    return prod_L
    #return tf.where(tf.math.is_nan(prod_L), tf.zeros_like(prod_L), prod_L)
...
class DensityModel :
    ...
    def sample (self, n_points, conditions, num_observables=None, verbose=True) :
        ...
        for obs_idx in range(1, num_observables) :
            ...
            XN = self.likelihood_models[obs_idx].sample(1, conditions, np.array(X1_to_XN).transpose()).reshape(n_points)
            X1_to_XN.append(XN)
