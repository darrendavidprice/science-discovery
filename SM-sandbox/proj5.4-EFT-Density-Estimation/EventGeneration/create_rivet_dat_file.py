...
#
def run_rivet (inputs, routine_name="event_parser", docker_image="hepstore/rivet:2.7.2", buildplugin=None) :
	cwd = os.getcwd()
	...
	if type(buildplugin) != type(None) :
		...
		stdout = client.containers.run("hepstore/rivet:2.7.2", 
									   f"rivet-buildplugin {buildplugin}", 
									   remove=True, 
...
def run_program () :
	args = parse_args()
	run_rivet(args.inputs, buildplugin="event_parser.cxx")
